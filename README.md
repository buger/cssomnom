...
- **`CSSImportRule.styleSheet`**: Returns the associated `CSSStyleSheet` object, never `null`. cssom-1 § 6.4.3 permits `null` here ("must return the associated CSS style sheet, if any, or null otherwise"); the non-null associated sheet is our documented offline posture (authorized 2026-08-23). Because the library operates as a static, offline parser without network or disk I/O, the href is never fetched: the associated sheet is exposed in its browser pre-load state — empty until a host supplies content offline via `replaceSync()`. Public linkage is live: `ownerRule` is the import rule and `parentStyleSheet` tracks the owning sheet.
