...
- [x] Structurally unpairable left mute (no ignore): L1077 `while (true)` F (literal). L1130 `name === '--'` T (dead after L1059 early return). L1107 `t1` F and L1110 `t2` F are unpairable through parseStyleSheet / cssText / parseDeclaration (tokenize + `ArrayComponentValueStream`/`LazyComponentValueStream` only yield objects); unique-cause T is driven via the shipped method (not a copied algorithm).
